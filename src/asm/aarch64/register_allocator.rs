...
use super::types::{
    Addr, IndexOperand, Operand, Register, RegisterClass, RegisterSize, SCRATCH0, SCRATCH1,
};
...
impl<'a> RegisterAllocator<'a> {
    ...
    fn allocate(&mut self) -> RegisterAllocation {
        ...
        for (class, pool) in [
            (RegisterClass::Gpr, ALLOCATABLE_REGS),
            (RegisterClass::Fpr, ALLOCATABLE_FPRS),
        ] {
            let class_present = filter_present_by_class(&present, &vreg_sizes, class, num_vregs);
            let mut graph =
                InterferenceGraph::build(self.insts, &liveness, &class_present, num_vregs);
            let Coloring { coloring, spilled } = graph.color(&pool);
...
fn filter_present_by_class(
    present: &Bitset,
    vreg_sizes: &HashMap<usize, RegisterSize>,
    class: RegisterClass,
    num_vregs: usize,
) -> Bitset {
    let mut filtered = Bitset::new(num_vregs);
    for v in present.iter() {
        let size = vreg_sizes.get(&v).copied().unwrap_or(RegisterSize::X64);
        if size.class() == class {
            filtered.insert(v);
        }
    }
...
impl InterferenceGraph {
    ...
    fn color(&mut self, colors: &[u8; NUM_COLORS]) -> Coloring {
        if self.present.is_empty() {
            return Coloring::empty();
        }

        let (stack, potential_spills) = self.simplify();
        self.select(stack, potential_spills, colors)
    }
    ...
    fn select(
        ...
        while let Some(v) = stack.pop() {
            ...
            if let Some(&color) = colors.iter().find(|c| used_colors & (1u32 << **c) == 0) {
...
impl<'a> InstRewriter<'a> {
    ...
    fn rewrite_fbinop(
        &mut self,
        op: crate::asm::aarch64::types::FBinOp,
        dst: Register,
        lhs: Register,
        rhs: Register,
    ) -> Result<(), Error> {
    ...
    fn rewrite_lea(&mut self, dst: Register, addr: &Addr) -> Result<(), Error> {
        ...
        self.write_to_dst(dst, RegisterSize::X64, dst_scratch, |final_dst| {
            Instruction::Lea {
                dst: final_dst,
                addr: addr_rewritten,
            }
